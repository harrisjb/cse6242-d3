...
Her we parse the jason and create csv file that can be used to preprocess our data to work with these maps
'''

# Loads the State Names and Codes into a Dictionary keyed by State Name
def load_state_codes():
    state_codes = {}
    with open('state_codes_by_name.csv') as csv_file:
...
def load_state_codes_and_ids():
    state_codes = {}
    with open('states-10m.csv') as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        for row in csv_reader:
...
def create_counties_csv():
    ...
    with open('counties-10m.csv', mode='w') as csv_file:
        csv_writer = csv.writer(csv_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
        counties_data.sort()
