...
pills_sold = FOREACH pills GENERATE Reporter_family, QUANTITY;

-- WHERE

-- GROUP BY
group_seller = GROUP pills_sold BY Reporter_family;
total_pills_by_seller = FOREACH group_seller GENERATE FLATTEN(group) as Reporter_family, SUM(pills_sold.QUANTITY) as TOTAL_PILLS;


-- ORDER BY
results = ORDER total_pills_by_seller BY TOTAL_PILLS DESC, Reporter_family ASC;
