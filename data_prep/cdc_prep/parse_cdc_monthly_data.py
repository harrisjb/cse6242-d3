...
BASE_DIR = './data'
# "Notes"	"Year"	"Year Code"	"Month"	"Month Code"	"County"	"County Code"	Deaths	Population	Crude Rate
# "Notes"	"County"	"County Code"	Deaths	Population	Crude Rate
def fetch_data(year):
    # Load the data to be processed
    file_name="{}/Mortality_Monthly_{}.txt".format(BASE_DIR,year)
...
def clean_data(data_set):
    data_set = data_set.drop(["Notes","Year","Year Code","Month","Population","Crude Rate"], axis=1)
    ...
    data_set["Year"] = data_set["Month Code"].apply(get_year)
    data_set["Month"] = data_set["Month Code"].apply(get_month)
    data_set = data_set.drop(["Month Code"], axis=1)
    return data_set

def clean_data1(data_set):
...
if __name__=='__main__':
    for year in range(2006,2013):
        ...
        file_name = "{}/Mortality_Monthly_{}.csv".format(BASE_DIR,year)
