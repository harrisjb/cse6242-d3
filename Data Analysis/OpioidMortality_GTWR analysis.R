...
boxplot(opioid_df$opioid_perperson)
boxplot(opioid_df$opioid_perperson~opioid_df$year)
...
global_association_pp <- lm(log_mortality ~ opioid_perperson + hhincome_clean + age_clean + percent_white, data = opioid_df)
summary(global_association_pp)
plot(global_association_pp, which = 1)
...
varlist <- c("fips", "name.x", "state.x", "county", "state.y", "year", "longitude", "latitude", 
             "deaths_clean", "mortality_rate", "log_mortality", "opioids_in_gm", "opioid_perperson",
...
analysis_data <- subset(analysis_data, !(is.na(deaths_clean)) & !(is.na(hhincome_clean)))
...
fixedbisquare_bw <- bw.gtwr(log_mortality ~ opioid_perperson + hhincome_clean + age_clean + percent_white, data=analysis_data,
                     obs.tv = analysis_data$year, approach = "AICc", kernel = "bisquare", adaptive = F, lamda = 0.05, t.units = "year", ksi = 0,
                      verbose = TRUE)

fixedgaussian_bw <- bw.gtwr(log_mortality ~ opioid_perperson + hhincome_clean + age_clean + percent_white, data=analysis_data,
...
gtwr_model <- gtwr(log_mortality ~ opioid_perperson + hhincome_clean + age_clean + percent_white, data=analysis_data, 
...
analysis_data$coef_opioids_perperson <- results$opioid_perperson
...
analysis_data$coef_opioids_per100_gm_pp <- results$opioid_perperson*100 # scaled coefficient to a 100 gm increase in yearly opiod
